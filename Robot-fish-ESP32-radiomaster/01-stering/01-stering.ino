...
#define CRSF_TX_PIN 22   // Radio RX -> ESP32 TX (opsional, diubah dari 27)
...
int pwmToServoAngle(int pwm) {
  // PWM 1000 = 0 derajat, PWM 2000 = 180 derajat
...
void setup() {
  ...
  Serial.println("==========================================");
  Serial.println("RadioMaster Pocket + Servo MG996R Ready!");
  Serial.println("YAW channel controls servo on GPIO27");
  Serial.println("==========================================");
}
...
void loop() {
  ...
  if (frameType == 0x16) {
    ...
    // Print data dengan info servo
    Serial.printf(
      "THR:%4d  ROLL:%4d  PITCH:%4d  YAW:%4d -> SERVO:%3d°  |  "
      "SA:%s SB:%s SC:%s SD:%s  SE:%s  S1:%4d\n",
      throttle, roll, pitch, yaw, servoAngle,
